...
def train_and_log_model() -> None:
    ...
    # mlflow.set_experiment("RandomForestRegressor-forecast-test-0")

    timestamp = datetime.now().strftime("%d_%m:%H_%M_%S")
    
    mlflow.set_experiment(f"call_volume_forecasting_{timestamp}")
    ...
    with mlflow.start_run(run_name="random_forest_claims_forecast"):
        ...
        mlflow.log_metric("mean_actual_volume", y_test.mean())
        mlflow.log_metric("mean_predicted_volume", predictions.mean())
        mlflow.log_metric("max_abs_error", np.max(np.abs(y_test - predictions)))
        ...
        feature_importance_path = OUTPUT_DIR / "feature_importances.csv"
